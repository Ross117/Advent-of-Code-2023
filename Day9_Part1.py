...
def get_nxt_val(diff_hist: list[int]) -> int:
    '''
    Return the next value in a sequence based on a given pattern of
    differences between values.
    '''

    return functools.reduce(lambda a, b: a + b, reversed(diff_hist))
...
class TestSolution(unittest.TestCase):
    ...
    def test_get_nxt_val(self):
        self.assertEqual(get_nxt_val([15, 3, 0]), 18)
